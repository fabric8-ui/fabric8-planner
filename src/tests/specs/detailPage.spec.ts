...
describe('Detail View test: ', () => {
  ...
  it('should link a workitem', async () => {
    let linkType = 'blocks',
      searchWorkItem = '20-Workitem_Title_20',
      workItemTitle20 = 'Workitem_Title_20';
    await planner.workItemList.openDetailPage(c.workItemTitle1);
    await planner.waitUntilUrlContains('detail');
    await planner.detailPage.ready();
    await planner.detailPage.addLink(linkType, searchWorkItem, workItemTitle20);
    await planner.quickPreview.linklistItem.untilTextIsPresent(workItemTitle20);
